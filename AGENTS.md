...
| 🤖 **Новый агент** | `arena/01a025a4-th08` | Интеграция наработок, обновление прогресса, синхронизация веток | активна (2026-08-21) |
...
24. **Дубль `// FUNCTION: th08 0xADDR` аннотаций** (два тела с одним адресом): reccmp пишет
    `[ERROR] Dropped duplicate address 0x…` и сравнивает ПЕРВОЕ аннотированное тело — настоящая
    функция сидит на 20-30% при визуально-идеальном коде. Охота:
    `grep -rhn "^// FUNCTION: th08" src/*.cpp | awk '{print $4}' | sort | uniq -d` + читай [ERROR]-
    строки вердикта. Лечение: удалить устаревшее тело (def в .cpp И decl в .hpp). (424a20.)
25. **f32-литералы — ТОЛЬКО из rdata-байт** (`objdump -s --start-address=0x4bNNNN resources/th08.exe`,
    LE-quad). Рукописные таблицы констант ГНИЮТ (мои «5.0/96.0/32.0» оказались 80.0/384.0/8.0 —
    стоило 2 циклов). И ULP: `0.70710678f`→0x3F3504F3, а оригинал 0x3F3504F2 = `0.7071067f` —
    последняя цифра решает; сверяй бит-паттерн через `struct.pack('<f', x)` при 93-99% «всё верно».
26. **Slot = порядок ТЕКСТУАЛЬНЫХ объявлений (первое = [ebp-4]), не присваиваний.** Приём
    «слот выше, но присваивается позже»: голый `T x;` в заголовке функции + поздняя инициализация +
    `#pragma var_order(...)` (FUN_004113a0: `AnmVm *vm;` / entry-init / CALL / vm-assign).
    Вызов-читалка: порядок инструкций в дизасме точно показывает, где была строка присваивания.
27. **operator-new регрессия −33 (21.08), механика и оправданные/осуждённые:** все 33 жертвы зовут
    `new`/delete и печатаются в verbose как `+call operator new` (без сигнатуры) при orig
    `operator new(unsigned int)`. ОПРАВДАНЫ: reccmp-версия (0.1.6 с 13.07), тулчейн-кэш (b693a88
    до 028c43d), csv-ряд `_DirectSoundCreate8@12` (у Рейму здорово), `#pragma function` (бисект
    e08a2e0: снятие ничего не дало, Convert при этом упал 100→36 — pragma вернула). ОСУЖДЁН батч C
    в Spellcard (placement-new ×16 в ctor → члены): возврат placement-news инкапсулирует CRT-имя
    на нашей стороне обратно (вердикт-подтверждение следует). **Структурные правки Spellcard.hpp
    по 16 членам опасны для CRT-naming — лечим placement-new идиомой, а не layout-правкой.**
28. **Порядок .bss/.data = порядок .obj в `cxx_sources` (scripts/configure.py)** — наш главный рычаг
    выравнивания BSS к оригиналу (цветовые константы 0xfff0e0 и ко, «g_X+OFFSET разъезд»).
    Дополняется дозаполнением недекомпилированных .data-таблиц по GAPDIFF-карте Санаэ.
...
- Ая (th10): её апдейты в `Swiizyu/th10` PR #2 — последний: mapping/CSV groundwork
  ...
  **21.08 вечер:** в th10 головой интеграции стала **Хина Кагияма** (ветка `arena/01a02386-th10`,
  PR #10, 10/57 на 100% — у них рекорд; тулчейн там /O2+/GL+/LTCG, ловушки другие: PCH /
  `#pragma optimize("gty", on)`, GS-cookie). К ним туда же пришёл помощник (`arena/01a02448-th10`).
  Ая жива, работает (4/40 → сотка TitleScreen::OnUpdate). Мост: читаем их PR #2, важное несём сюда.
