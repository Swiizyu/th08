...
def create_status_profile():
    ...
    # Try to use reccmp-functions.csv for counting
    try:
        reccmp_funcs = get_file(script_path / "config" / "reccmp-functions.csv")
        num_funcs = 0
        for line in reccmp_funcs:
            parts = line.strip().split(",")
            if len(parts) >= 3 and parts[2] == "function":
                num_funcs += 1
        
        # Use mapping.csv for byte counting
        ...
        raw_func_percentage = len(impl) / num_funcs * 100 if num_funcs > 0 else 0
        byte_impl_percentage = impl_bytes / total_func_bytes * 100 if total_func_bytes > 0 else 0
        
        return raw_func_percentage, byte_impl_percentage
    except:
        # Fallback to original mapping.csv-based counting
        maps = get_file(script_path / "config" / "mapping.csv")
        total_func_bytes = 246112  # number of function bytes
        impl_bytes = 0
        num_funcs = 0
...
def get_unimplemented():
    ...
    for f_name, location, *unused in (line.split(",") for line in maps):
        if (
            int(location.removeprefix("0x"), 16) > 4680946
        ):  # 0x00476cf2 is where 3rd party lib functions start
            break
